...
**v0.1.0 — выпущена 8 сентября 2026 для ежедневного использования одним владельцем на этом Mac.**
[Состав релиза, проверка и ограничения](docs/releases/v0.1.0.md).

[Сон, еда и тренировки — как пользоваться](docs/three-jobs-pilot.md).

Быстрые ориентиры: [сценарии — работает / отложено / изначальный бэклог](docs/scenarios.md)
и [расположение папок проекта](docs/workspace-layout.md).

Дальнейшее развитие: [продуктовый бэклог короткими кейсами](docs/backlog.md).
...
Установленная сокращённая v0.1: один пользователь, история ключевых анализов,
WHOOP, Telegram и сохранение новых документов. Calendar, второй пользователь и
расширенные напоминания перенесены в [бэклог](docs/backlog.md); уже написанный код
сохранён. [Согласованная граница запуска](docs/superpowers/specs/2026-09-06-v01-launch-boundary-design.md).
...
[отчёте установленной версии](docs/v01-results-2026-09-08.md).
...
ответы врача. Обычный ответ короткий: вывод, важные факты и следующий шаг,
без технического списка источников. Внутренние ссылки проверяются до отображения;
заключение в архиве не считается подтверждением актуального диагноза.
Каждый вопрос самостоятельный: память предыдущих сообщений пока не подключена.
