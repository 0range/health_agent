...
def provenance(
    profile_id: str = PROFILE_A, account_id: str = "personal"
...
def test_streams_to_profile_account_isolated_vault(tmp_path: Path) -> None:
    ...
    source.write_bytes(b"medical-pdf")
    prepared = PreparedAttachment(
        source,
        hashlib.sha256(b"medical-pdf").hexdigest(),
        len(b"medical-pdf"),
        "application/pdf",
    )
    ...
    assert not (tmp_path / "tmp" / PROFILE_A / "gmail" / "personal").exists()
...
def test_rejects_cross_profile_or_account_content(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="another profile"):
        importer.import_attachment(
            provenance(PROFILE_B),
            PreparedAttachment(tmp_path / "x", "a", 1, "application/pdf"),
        )
    with pytest.raises(ValueError, match="another account"):
        importer.import_attachment(
            provenance(PROFILE_A, "work"),
            PreparedAttachment(tmp_path / "x", "a", 1, "application/pdf"),
        )
