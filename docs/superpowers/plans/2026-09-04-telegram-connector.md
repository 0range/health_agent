...
- [ ] Test atomic `0600` token storage, UUID/user uniqueness, profile-separated identity lookup, durable offsets/update claims, attachment audit without content, and outbound delivery reservations.
- [ ] Implement the contracts and private stores with idempotent schema initialization and no dialogue/content columns.
- [ ] Run `uv run pytest tests/telegram/test_stores.py -q` and commit.
...
- [ ] Test exact `getUpdates` offset/timeout/allowed updates, webhook detection, `getFile`, streamed download without token leakage, 429 `retry_after`, 5xx retry, non-retryable 4xx, and `sendMessage` payload.
- [ ] Implement Bot API calls with sanitized exceptions and bounded retries; never expose the token-bearing endpoint.
- [ ] Test deterministic text splitting, profile/chat lookup, reminder delivery keys, and repeat-send suppression.
- [ ] Implement the messenger and run focused tests; commit.
...
- [ ] Test unknown users and non-private/channel/bot senders are silently ignored with no downstream call or reply.
- [ ] Test `/help`, `/status`, `/sync`, open text questions with profile/time/message context, and no persisted text.
- [ ] Test PDF/photo/voice metadata and streamed bytes, size/hash/provenance audit, oversized-file status, same update replay, same bytes across two profiles, and offset recovery after a completed update.
- [ ] Implement conservative update parsing/routing and durable terminal offset advancement; run focused tests and commit.
...
- [ ] Test hidden token entry, profile existence validation, bind/unbind/status truthfulness, and output free of tokens/message content.
- [ ] Add settings and CLI wiring without a fake question-answer runtime.
- [ ] Document official @BotFather setup, safe numeric-ID discovery/binding, long-poll limitation, file limit, exact-once caveat, and the missing real-agent/inbox runtime composition.
- [ ] Run full pytest, Ruff, mypy, `alembic upgrade head`, `git diff --check`, and credential-pattern checks; write the factual report and commit.
