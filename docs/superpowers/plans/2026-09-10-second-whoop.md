...
- [x] Verify both authorizations match the second profile, full-sync public resources, configure private manifest, sync detailed resources, and run research check. Confirm automatic refresh, launchd configuration and real per-person row counts. Update runbook, commit and push reviewed changes.

Validation: 79 related tests passed; Ruff and mypy passed. Live app-session identity and renewal verified. Both public OAuth and app-session identity and refresh are verified. Full public backfill and regular detailed collection completed without errors; automatic discovery includes two public accounts and hourly detail targets. The quality report returns ok for both participants and the shared sensor.
