...
- User photo grouping extension: gap <40minutes from latest confirmed photo of current meal means same meal; gap >150minutes means new meal; inclusive40..150minutes requires one clarification. Compare actual Telegram event timestamps, not end estimates. Preserve every photo and ambiguous pending input. Reminder anchor/end estimate = latest confirmed photo +20minutes, not a later text comment. Store `ended_at` with `end_source=last_photo_plus_20m`. Do not impose a one-hour maximum meal length.
...
- [ ] In PilotBrain._profile_context, add `recorded_food_history` using14days only when domain==sleep. Include separately labelled `selected_food_framework` from own food/settings/protocol with only interval_hours, allowed_interval_hours, plate_rules, preferences, uncertainties, source, not_universal_medical_rules; bound lists/strings, exclude arbitrary fields. This is the chosen user framework, not scientific evidence. Shared goals/Apple/source priorities remain intact. Extend rules: use facts as logs, not complete intake; never infer cause of weight/sleep changes from simultaneous logs alone; disclose absent data. No photo transfer or other-domain conversation copy.
...
- [ ] Plain-text discussion keeps latest proposal/accepted plan context. Explicit change requests (`перенеси`, `замени`, `поменяй`, `скорректируй` referring to plan/session) save user turn first and generate a revised proposal using current proposal plus request/history; do not mutate accepted plan. A question such as `почему бег во вторник?` is discussion only. Clear constraints `в среду не могу` stay in dialogue and inform later draft; acknowledge as reported constraint, not secretly accepted schedule. Natural request `покажи план` displays latest proposal, `давай план на неделю` generates; slash commands stay compatible. Tests ordinary discussion→explicit reschedule→versioned draft, no implicit acceptance, idempotent update and current proposal context.
