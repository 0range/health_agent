...
Один управляемый лист ограничен 50 000 строк и 500 000 ячеек за синхронизацию:
больший срез останавливается безопасно, без неограниченного запроса в Google.
...
Если создание таблицы могло завершиться в Google, но ответ потерялся, следующие
запуски не создают дубликат. После ручной проверки Drive разрешите новую попытку:
`uv run health-agent sheets configure PROFILE_UUID --reset-unknown-creation`.
...
Синхронизация различает исходный тип ячейки: настоящий Google Sheets formula
в `Needs review` отклоняется, а текст, начинающийся с `=`, допустим в исходных
неизменяемых колонках только при точном совпадении с доверенной строкой из базы.
Поля решения и исправлений не принимают строки, похожие на формулы.
